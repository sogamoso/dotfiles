...
local appWatcher = hs.application.watcher.new(function(name, event, app)
	...
	local focusedWorkspace = hs.execute("/opt/homebrew/bin/aerospace list-workspaces --focused 2>/dev/null")
		:match("(%S+)")
	...
	hs.task
		.new(
			...
			nil,
			{ "--trigger", "aerospace_workspace_change", "FOCUSED_WORKSPACE=" .. workspace }
		)
		:start()
end)
